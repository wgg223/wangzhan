...
**表格自动识别链接与安全跳转**
- 新增：整格 `http://` / `https://` / `ftp://` 纯文本 URL 自动转为可点击超链接（Univer 原生蓝色下划线样式）；打开文档全量扫描存量文本，数据动态更新（本地编辑 / AI 写入 / 程序化赋值 / 撤销重做 / 远端同步）后自动重扫
- 防误触：严格整格匹配 + `new URL()` 双重校验（无空白、协议合法、域名存在），「请访问 https://x.com」等混合文本不误转
- 跳转：站内链接（域名一致）免确认直接新窗口打开；站外链接弹出安全确认框（⚠ 图标 + 「这是外部链接，可能存在安全风险，是否继续访问？」+ 目标域名/完整地址 + 取消/确定），确认后 `noopener,noreferrer` 打开
- `ftp://` 协议：包装 Univer 链接解析服务实例，绕过其内置 http/https/mailto 白名单拦截；锁定（只读）模式下识别与跳转照常（仅本地展示不回写）

**修复既有静默失效（程序化写入路径）**
- 修复：此 Univer 版本 `sheet.command.set-range-values` 依赖当前选区、忽略 `cellValue` 参数——链接转换 / AI 公式写入 / 远端同步三处改走 `sheet.mutation.set-range-values` mutation
- 修复：FWorkbook 门面无 `syncExecuteCommand` 方法，远端单元格同步此前一直静默异常并回退「整页重载」兜底；评论定位跳转所用选区命令不存在，改用 `sheet.operation.set-selections`
