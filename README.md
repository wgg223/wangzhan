...
- **在线表格**（Luckysheet 编辑器，功能接近 Excel：多 Sheet、公式、图表、条件格式；实时在线用户、锁定控制、自动保存 / Ctrl+S；实时协同编辑（2 秒增量同步、编辑者角标、单元格编辑历史）；CSV / Excel 导入，自动识别中文编码与分隔符；文档级权限：查看 / 编辑 / 下载 / 创建副本，权限申请与审批、站内通知）
...
- **在线表格管理**（创建 / 编辑 / 删除、Excel 多 Sheet 批量导入自动建表、列管理）
- 系统设置（基础/SMTP/协议/弹窗）
