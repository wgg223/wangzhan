...
function checkUpdateNotification() {
  fetch('/admin/maintenance/update-status', {
    headers: {
    ...
    .then(function(result) {
      if (result.success && result.data.hasUpdate && !result.data.alreadyNotified) {
        showUpdateNotification(result.data);
      }
    })
    .catch(function(err) {
      console.error('Check update notification failed:', err);
    });
}
...
function showUpdateNotification(data) {
  ...
  overlay.innerHTML = '<div class="update-notification">' +
    '<div class="update-notification-header">' +
      '<span class="update-icon">🔄</span>' +
      '<h3>发现新版本 v' + escapeHtml(data.latestVersion) + '</h3>' +
    '</div>' +
    '<div class="update-notification-body">' +
      (data.releaseName ? '<h4>' + escapeHtml(data.releaseName) + '</h4>' : '') +
      (data.releaseBody ? '<div class="release-notes">' + escapeHtml(data.releaseBody).replace(/\n/g, '<br>') + '</div>' : '') +
      (data.publishedAt ? '<p class="published-at">发布时间: ' + new Date(data.publishedAt).toLocaleString('zh-CN') + '</p>' : '') +
    '</div>' +
    '<div class="update-notification-actions">' +
      '<button class="btn btn-primary" onclick="performAutoUpdate(this)">立即更新</button>' +
      '<button class="btn btn-secondary" onclick="dismissUpdateNotification()">稍后提醒</button>' +
    '</div>' +
  '</div>';
  ...
  if (!document.getElementById('update-notification-styles')) {
    ...
    style.textContent = '.update-notification-overlay{position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);display:flex;align-items:center;justify-content:center;z-index:10000;animation:fadeIn 0.3s ease}' +
      '.update-notification{background:#fff;border-radius:12px;padding:24px;max-width:500px;width:90%;max-height:80vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,0.3)}' +
      '.update-notification-header{display:flex;align-items:center;gap:12px;margin-bottom:16px}' +
      '.update-icon{font-size:32px}' +
      '.update-notification-header h3{margin:0;font-size:20px;color:#1f2937}' +
      '.update-notification-body{margin-bottom:20px}' +
      '.update-notification-body h4{margin:0 0 12px 0;color:#374151;font-size:16px}' +
      '.release-notes{background:#f3f4f6;padding:12px;border-radius:8px;font-size:14px;line-height:1.6;color:#4b5563;max-height:200px;overflow-y:auto;white-space:pre-wrap}' +
      '.published-at{font-size:13px;color:#9ca3af;margin:8px 0 0 0}' +
      '.update-notification-actions{display:flex;gap:12px;justify-content:flex-end}' +
      '.update-notification-actions .btn{padding:10px 20px;border:none;border-radius:8px;font-size:14px;cursor:pointer;font-weight:500}' +
...
function performAutoUpdate(btn) {
  btn.disabled = true;
  btn.textContent = '更新中...';

  fetch('/admin/maintenance/auto-update', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'CSRF-Token': getCsrfToken(),
...
function dismissUpdateNotification() {
  var overlay = document.querySelector('.update-notification-overlay');
  if (overlay) {
    overlay.remove();
  }

  // Mark as notified
  fetch('/admin/maintenance/mark-notified', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'CSRF-Token': getCsrfToken(),
...
// Check for updates when admin page loads
document.addEventListener('DOMContentLoaded', function() {
  // Only check on main admin pages, not on maintenance page itself
  if (!window.location.pathname.includes('/admin/maintenance')) {
    setTimeout(checkUpdateNotification, 2000);
  }
});
